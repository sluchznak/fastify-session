  ...
  if (decryptedSessionId === false) {
    ...
    options.store.get(decryptedSessionId, (err, session) => {
      ...
      request.session = new Session(
        ...
        cookieOpts,
        secret,
        { ...session, encryptedSessionId: sessionId, sessionId: decryptedSessionId }
      )
      done()
    })
  }
...
function onRequest (options) {
  ...
  return function handleSession (request, reply, done) {
    ...
    if (sessionId && options.encryptedSessionIdPrefix && sessionId.startsWith(options.encryptedSessionIdPrefix)) {
      sessionId = sessionId.replace(new RegExp(`^${options.encryptedSessionIdPrefix}`), '')
    }
...
function onSend (options) {
  return function saveSession (request, reply, payload, done) {
    ...
    options.store.set(session.sessionId, session, (err) => {
      ...
      reply.setCookie(
        options.cookieName,
        options.encryptedSessionIdPrefix ? `${options.encryptedSessionIdPrefix}${session.encryptedSessionId}` : session.encryptedSessionId,
...
function ensureDefaults (options) {
  ...
  options.encryptedSessionIdPrefix = options.encryptedSessionIdPrefix || ""
